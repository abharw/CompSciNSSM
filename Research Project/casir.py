...
def screenplay(titile,cellCountX, cellCountY, info, total_iteration):
    ...
    playtime = 0
    cycletime = 0
    interval = 1.5    #.15 how long one single images should be displayed in seconds
    picnr = 0
...
pl.plot(list(map(itemgetter(4), RES)), list(map(itemgetter(3), RES)),'green', label='Recovered')
pl.plot(list(map(itemgetter(4), RES)), list(map(itemgetter(1), RES)), 'blue',label='Susceptibles')
pl.plot(list(map(itemgetter(4), RES)), list(map(itemgetter(2), RES)), 'red',label='Infected')
pl.legend(loc=0)
